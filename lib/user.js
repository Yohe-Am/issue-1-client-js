...
function NewUserServiceClient(baseURL) {
    let client = {
         ...
         */
        async addPicture(username, authToken, imageData, imageName = 'client-js.jpg') {
            return addPicture(client.baseURL, username, authToken, imageData, imageName);
        },
        /**
         * Removes the user's profile picture.
         */
        async removePicture(username, authToken) {
            return removePicture(client.baseURL, username, authToken);
        },
    };
    return client;
}
 ...
 */
async function addPicture(baseURL,
                          username, authToken,
    ...
    return (await makeRequest(
        `${baseURL}/users/${username}/picture`,
        {
            ...
            ...await attachImageToRequest(
                imageData,
                attachAuthTokenToHeader(authToken),
                imageName
            ),
        }))
        .data;
 ...
 * @param {string} authToken
 */
async function removePicture(baseURL, username, authToken) {
    return (await makeRequest(
        `${baseURL}/users/${username}/picture`,
        {
            method: 'delete',
